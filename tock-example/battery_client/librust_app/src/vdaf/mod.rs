...
impl From<getrandom::Error> for VdafError {
...
impl From<FlpError> for VdafError {
    fn from(value: FlpError) -> Self {
        Self::Flp(value.to_string())
    }
}
...
pub trait Vdaf: Clone + Debug {
    ...
    type AggregateShare: Aggregatable<OutputShare = Self::OutputShare> + Into<Vec<u8>>;
    //+ for<'a> TryFrom<&'a [u8]>
    //+ Into<Vec<u8>>;
...
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct OutputShare<F>(pub Vec<F>);
...
pub struct AggregateShare<F>(pub Vec<F>);
