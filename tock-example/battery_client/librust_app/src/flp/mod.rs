...
impl core::fmt::Display for FlpError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        write!(f, "{:?}", self)
    }
}

impl Error for FlpError {}

impl From<FftError> for FlpError {
    ...
    fn decide(&self, verifier: &[Self::Field]) -> Result<bool, FlpError> {
        ...
        for idx in 0..gadgets.len() {
            ...
            if e != verifier[verifier_len + next_len - 1] {
                println!("e: {}, other: {}", e, verifier[verifier_len + next_len - 1]);
                return Ok(false);
            }
